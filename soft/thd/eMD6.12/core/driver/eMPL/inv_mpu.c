...
/* pp加入为了可以分离出i2c读取并使用中断方式(增加并行度) */
void pp_get_compass_mag_sens_adj(short *adj)
{
    adj[0] = st.chip_cfg.mag_sens_adj[0];
    adj[1] = st.chip_cfg.mag_sens_adj[1];
    adj[2] = st.chip_cfg.mag_sens_adj[2];
}
